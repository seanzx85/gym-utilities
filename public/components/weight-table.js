...
class WeightTable extends React.Component {
  ...
  updateTableWeight = weight => {
    ...
    for (let i = 0; i < arr.length; i += 1) {
      arr[i] =
        Math.round((i + 1 + this.state.startOffset) / 20 * weight * 100) / 100;
    }
    return arr;
  };
  ...
  render() {
    return (
      <div className="weight-table max-width-mobile">
        ...
        {this.state.calcWeights.map((weight, index) => {
          return (
            <div className="row col3" key={index}>
              <div className="cell">
                {(index + 1 + this.state.startOffset) * 5}%
              </div>
              <div className="cell">{weight}</div>
